...
const getDashboardStats = async (req, res) => {
  try {
    ...
    if (user.role.toLowerCase() === "student") {
      const totalEnrolledModules = await modules
        .find({ userId: user._id })
        .countDocuments();
      const completedModules = await modules
        .find({ status: "completed", userId: user._id })
        .countDocuments();
      const activeModules = await modules
        .find({ status: "active", userId: user._id })
        .countDocuments();
      ...
      enrollments.forEach((enrollment) => {
        ...
        const timeSpentInModule = enrollment.studySessions.reduce(
      ...
      const instructorModules = await Module.find({ author: user._id }).select(
        "_id"
      );
